...
class AuthService {
  ...
  Future<bool> login(String username, String password) async {
    try {
      final response = await http.post(
        Uri.parse('https://localhost:7164/api/User/login'), // Replace with production URL
  ...
  Future<Map<String, dynamic>> register(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('https://localhost:7164/api/User/register'), // Replace with production URL
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
      ...
      if (response.statusCode == 201) {
        ...
        final errorMessage = _parseErrorResponse(response.body) ?? 'Registration failed';
