...
Future<void> _fetchCreatorDetails() async {
 ...
 Future<void> _deleteSubtopic() async {
  ...
  Future<bool> _isOwnSubtopic() async {
    final userIdStr = await _authService.getUserId();
    final loggedInUserId = int.tryParse(userIdStr ?? '');
    final subtopicUserId = widget.subtopic['userId'];

    debugPrint('Checking ownership: loggedInUserId=$loggedInUserId, subtopicUserId=$subtopicUserId');

    if (loggedInUserId == null || subtopicUserId == null) {
      return false;
    }

    return loggedInUserId == subtopicUserId;
  }
