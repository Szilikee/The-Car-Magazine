...
Future<void> _checkLoginStatus() async {
  ...
  if (loggedIn && mounted) {
    ...
    Navigator.pushReplacement(
      context,
      MaterialPageRoute(builder: (context) => MainPage(authService: _authService)),
    );
  }
}
      ...
      if (response.statusCode == 200) {
        ...
        if (mounted) {
          setState(() {
            ...
            _userId = userId;
            _profileImageUrl = responseData['profileImageUrl'];
            _isLoggedIn = true;
          });
        }
      } else {
...
Future<void> _registerUser() async {
  ...
  try {
    ...
    if (result['success'] == true) {
      ...
      showFailed(context, result['error'] ?? 'Registration failed');
    }
  } on SocketException {
    showFailed(context, 'No internet connection');
  } catch (e) {
    showFailed(context, 'Registration failed: $e');
  }
}
 ...
 Future<void> _verifyCode(String code) async {
  ...
  if (response.statusCode == 200) {
    ...
    if (mounted) {
      Navigator.pop(context); // Bezárja a verifikációs dialógust
      setState(() {
        _isRegistering = false;
      });
      showSuccess(context, translations[widget.selectedLanguage]!['verificationSuccess']!);
      await _checkLoginStatus(); // Frissíti az állapotot és navigál a MainPage-re
    }
  } else {
    String errorMessage;
    try {
 ...
 void _showVerificationDialog() {
  showDialog(
    context: context,
    barrierDismissible: true, // Engedélyezi a dialógus bezárását a háttérre kattintással
    builder: (context) {
        ...
        title: Row(
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () {
                Navigator.pop(context); // Bezárja a dialógust az X gombra kattintva
              },
            ),
          ],
        ),
    ...
    body: Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 20),
          Text(
            translations[widget.selectedLanguage]!['loading'] ?? 'Logging in...',
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                if (mounted) {
                  ...
                  showSuccess(context, t['logoutSuccess'] ?? 'Logged out successfully');
                }
              },
